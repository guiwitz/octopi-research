...
print('Imported multipoint_custom_script_entry_v2_towbin.py')
...
def acquire_image_towbin(worker, current_path, region_id, fov):

    image_multichannel = None
    ...
    for z_level in range(worker.NZ):
        ...
        worker.signal_register_current_fov.emit(worker.stage.get_pos().x_mm, worker.stage.get_pos().y_mm)
        ...
        # update FOV counter
        worker.af_fov_count = worker.af_fov_count + 1
...
def perform_autofocus(worker, region_id):
    if worker.do_reflection_af:
        ...
        perform_contrast_autofocus(worker, region_id, no_condition=True)
...
def perform_contrast_autofocus(worker, region_id, no_condition=True):
    if no_condition or (
        ...
        #    worker.scan_coordinates_mm[region_id][2] = worker.stage.get_pos().z_mm
...
def update_widget_z_level(worker, region_id):
    ...
    if worker.microscope.flexibleMultiPointWidget.location_list is not None:
        try:
            worker.microscope.flexibleMultiPointWidget.update_location_z_level(pos_numerical_id, worker.stage.get_pos().z_mm)
        except:
            print("Failed to update flexible widget z")
        '''try:
            worker.microscope.multiPointWidgetGrid.update_region_z_level(
                region_id, worker.stage.get_pos().z_mm
            )
        except:
...
def get_autofocus_config(worker):
    ...
    return next(
        (config for config in worker.multiPointController.channelConfigurationManager.get_channel_configurations_for_objective(
                    worker.multiPointController.objectiveStore.current_objective
