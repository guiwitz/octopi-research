...
class TowbinWidget(QWidget):
    ...
    def update_position(self):
        ...
        x = self.parent.stage.get_pos().z_mm#self.parent.navigationController.x_pos_mm
        y = self.parent.stage.get_pos().y_mm#self.parent.navigationController.y_pos_mm
        z = self.parent.stage.get_pos().z_mm#self.parent.navigationController.z_pos_mm
    ...
    def copy_z_to_same_id(self):
        ...
        z_value = self.parent.table_location_list.item(row, 2).text()
        print(f'Z value to copy: {z_value}')
        ...
        for row in range(self.parent.table_location_list.rowCount()):
            ...
            if self.parent.table_location_list.item(row, 3).text().split('-')[0] == id:
                print(f'Row {row} matches group ID {id}, copying Z value')
                item = self.parent.table_location_list.item(row, 2)
                item.setText(z_value)